...
#Se minha senha for menor que 6 digitos/Se for menor retorna falso
    if len(password) < 6:
        messages.add_message(request, constants.ERROR, 'Sua senha deve conter 6 ou mais caractertes')
        return False

# Se minha senha for igual a confirmação da senha/ Se não for for (if not) retorna falso
    if not password == confirm_password:
        messages.add_message(request, constants.ERROR, 'As senhas não coincidem!')
        return False

# Procura caracteres de a - z maiusculo dentro d minha senha/ Se não(if not) conter retorna falso
    if not re.search('[A-Z]', password):
        messages.add_message(request, constants.ERROR, 'Sua senha não contem letras maiúsculas')
        return False

# Procura caracteres de a - z minusculo dentro d minha senha/ Se não(if not) conter retorna falso
    if not re.search('[a-z]', password):
        messages.add_message(request, constants.ERROR, 'Sua senha não contem letras minúsculas')
        return False

# Procura caracteres de 1 - 9 dentro da minha senha/ Se não(if not) conter retorna falso    
