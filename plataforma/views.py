    ...
    if request.method == "GET":
        ...
        try:
            paciente = Pacientes(nome=nome,
                                ...
                                telefone=telefone,
                                nutri=request.user)
                                # Nutricionista desse cliente, traz o usuário que esta logado(request.user)
...
def dados_paciente(request, id):
    ...
    if request.method == "GET":
        # Buscar os dados do cliente pelo methodo GET
        dados_paciente = DadosPaciente.objects.filter(paciente=paciente)
        # enviar os dados do paciente (dados_paciente) acrescentando no dicionario "{}" abaixo
        return render(request, 'dados_paciente.html', {'paciente': paciente, 'dados_paciente': dados_paciente})
    # Se o tipo da requisição for POST eu vou capituras esses dados(peso, altura,gordura...)
    elif request.method == "POST":
        ...
        return redirect('/dados_paciente/')
        # Necessário armazenar os dados do cliente na pasta plataforma models

# somente usuários logados podem fazer a requisição para esse url        
@login_required(login_url='/auth/login/')
@csrf_exempt # requisição via javascript
def grafico_peso(request, id):
    paciente = Pacientes.objects.get(id=id)
    dados = DadosPaciente.objects.filter(paciente=paciente).order_by("data")
                                                            # odernar pela data
    # criar uma lista   
    # Criando uma API (linhas 102 a 112) retornando uma jsonResponse                                                               #                             
